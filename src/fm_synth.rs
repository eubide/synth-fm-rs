...
impl Voice {
    ...
    pub fn process(&mut self, algorithm: u8, pitch_bend: f32, pitch_bend_range: f32, portamento_time: f32) -> f32 {
        if !self.active {
            return 0.0;
        }
...
impl FmSynthesizer {
    ...
    pub fn note_on(&mut self, note: u8, velocity: u8) {
        ...
        if self.mono_mode {
            ...
            if let Some(&voice_idx) = self.held_notes.get(&note) {
                self.voices[voice_idx].trigger(note, velocity_f, self.master_tune, self.portamento_enable && self.mono_mode);
                return;
            }
    ...
    pub fn process(&mut self) -> f32 {
        ...
        for voice in &mut self.voices {
            if voice.active {
                let voice_output = voice.process(self.algorithm, self.pitch_bend, self.pitch_bend_range, self.portamento_time);
                output += voice_output;
                _active_voices += 1;
            }
        }
